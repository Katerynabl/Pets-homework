...
// 5) Update pet with id ${pet.id} using form data
it('Update pet', () => {
  ...
  cy.request('GET', `/pet/${pet.id}`).then( response => {
    expect(response.status).to.be.equal(200);
     expect(response.body.id).to.be.equal(FormData.petId);
     expect(response.body.name).to.be.equal(FormData.name);
     expect(response.body.status).to.be.equal(FormData.status);
  })
})


// 6) Delete pet with id ${pet.id}
it('Delete by id', () => {
...
// 6.1 Get pet by id after deleting
it('Get pet by id after deleting', () => {
  cy.log(`Get pet with id deleting: ${pet.id}`);
  cy.request({ 
    method: 'GET',
    url: `/pet/${pet.id}`,
